...
export default function HomePage() {
  ...
  const sec = useSections("home");

  const hero = settings?.hero_content;
  const contact = settings?.contact_info;
  const companyStats = settings?.company_stats;

  // Section content from CMS
  const heroSec = sec.getContent("hero");
  const statsSec = sec.getContent("stats");
  const whySec = sec.getContent("why_choose");
  const ctaSec = sec.getContent("cta");
  const heroButtons = sec.getButtons("hero");
  const featuredButtons = sec.getButtons("featured_projects");
  const ctaButtons = sec.getButtons("cta");

  // Why choose features from CMS or fallback
  const whyFeatures = (whySec?.features as { title: string; desc: string }[] | undefined) ?? [
  ...
  // Stats items from CMS or fallback
  const statsItems = (statsSec?.items as { label: string; value: string; icon: string }[] | undefined) ?? [
    { label: "Active Projects", value: String(stats?.totalProjects ?? companyStats?.active_projects ?? 3), icon: "building" },
    { label: "Available Plots", value: String(stats?.availablePlots ?? 0), icon: "home" },
    { label: "Developed", value: companyStats?.sqft_developed ?? "10M+ Sqft", icon: "trending" },
    { label: "Happy Families", value: companyStats?.happy_families ?? "1000+", icon: "check" },
  ];

  const statIcons: Record<string, React.ReactNode> = {
    building: <Building2 size={20} className="text-red-600" />,
    home: <Home size={20} className="text-red-600" />,
    trending: <TrendingUp size={20} className="text-red-600" />,
    check: <CheckCircle size={20} className="text-red-600" />,
  };
  ...
  return (
    <div className="min-h-screen flex flex-col">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-36">
          <div className="max-w-2xl">
            ...
            <div className="inline-flex items-center gap-2 bg-red-600/20 border border-red-500/30 rounded-full px-4 py-1.5 text-red-300 text-sm mb-6">
              <CheckCircle size={14} />
              <span>{(heroSec?.badge as string) ?? hero?.badge ?? ""}</span>
            </div>
            )}
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold leading-tight mb-4">
              {sec.getTitle("hero") || hero?.title || ""}
            </h1>
            <p className="text-lg text-gray-300 mb-4 leading-relaxed">
              {sec.getSubtitle("hero") || hero?.subtitle || ""}
            </p>
            ...
            <div className="flex flex-wrap gap-3">
              {heroButtons.length > 0 ? heroButtons.map((btn, i) => (
                <Link key={i} href={btn.link}
                  className={`inline-flex ${btn.variant === "primary" ? "bg-red-600 hover:bg-red-700 text-white" : btn.variant === "secondary" ? "border border-white/30 hover:border-white text-white" : "border-2 border-red-600 text-red-600 hover:bg-red-600 hover:text-white"} px-6 py-3 rounded font-semibold items-center gap-2 transition-colors`}>
                  {btn.text} {btn.variant === "primary" && <ArrowRight size={16} />}
                </Link>
              )) : (
                <>
            ...
            {(heroSec?.starting_price || hero?.starting_price) && (
            <div className="mt-8 flex items-center gap-2 text-sm text-gray-400">
              <span className="bg-green-500 rounded-full w-2 h-2 inline-block"></span>
              {(heroSec?.starting_price as string) ?? hero?.starting_price ?? ""}
            </div>
            )}
          </div>
        </div>
            ...
            {statsItems.map((stat, i) => (
              <div key={i} className="flex flex-col items-center gap-1">
                <div className="w-10 h-10 bg-red-50 rounded-full flex items-center justify-center mb-1">
                  {statIcons[stat.icon] ?? <CheckCircle size={20} className="text-red-600" />}
                </div>
      ...
      {/* Featured Projects */}
      {sec.isActive("featured_projects") && (
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            <p className="text-red-600 text-sm font-semibold uppercase tracking-wider mb-2">Our Portfolio</p>
            <h2 className="text-3xl font-bold text-gray-900">{sec.getTitle("featured_projects") || "Featured Projects"}</h2>
            <p className="text-gray-500 mt-2">{sec.getSubtitle("featured_projects") || "TMA Approved housing societies in prime locations of Rahim Yar Khan"}</p>
          </div>
          ...
          <div className="text-center mt-8">
            {featuredButtons.length > 0 ? featuredButtons.map((btn, i) => (
              <Link key={i} href={btn.link}
                className={`inline-flex items-center gap-2 ${btn.variant === "outline" ? "border-2 border-red-600 text-red-600 hover:bg-red-600 hover:text-white" : "bg-red-600 hover:bg-red-700 text-white"} px-6 py-2.5 rounded font-semibold transition-colors no-underline`}>
      ...
      {/* Why Choose Us */}
      {sec.isActive("why_choose") && (
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <p className="text-red-600 text-sm font-semibold uppercase tracking-wider mb-2">Why Choose Us</p>
              <h2 className="text-3xl font-bold text-gray-900 mb-4">{sec.getTitle("why_choose") || "Rahim Yar Khan's Most Trusted Developer"}</h2>
              <p className="text-gray-600 mb-6">{(whySec?.description as string) ?? "Ravi Developers has been committed to excellence in Rahim Yar Khan. We have delivered over 10 million square feet of development to thousands of satisfied customers."}</p>
            ...
            <div className="relative">
              <img
                src={sec.getImages("why_choose")[0] || PLACEHOLDER_IMAGE}
      ...
      <section className="bg-gray-900 py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-2xl sm:text-3xl font-bold text-white mb-3">
            {sec.getTitle("cta") || "Secure Your Plot Today — Limited Availability"}
          </h2>
          <p className="text-gray-300 mb-6">{(ctaSec?.description as string) ?? hero?.starting_price ?? "Plots starting from PKR 3.75 Lac per Marla. 4-Year easy installment plan available."}</p>
          <div className="flex flex-wrap justify-center gap-4">
            {ctaButtons.length > 0 ? ctaButtons.map((btn, i) => (
              btn.variant === "primary" ? (
                <Link key={i} href={btn.link} className="inline-flex bg-red-600 hover:bg-red-700 text-white px-8 py-3 rounded font-semibold transition-colors no-underline">
                  {btn.text}
                </Link>
              ) : (
                <a key={i} href={btn.link} className="flex items-center gap-2 border border-white/30 hover:border-white text-white px-8 py-3 rounded font-semibold transition-colors">
                  <Phone size={16} /> {btn.text}
                </a>
              )
            )) : (
              <>
      ...
      {/* Testimonials */}
      {sec.isActive("testimonials") && (testimonials ?? []).length > 0 && (
        <section className="py-16 bg-gray-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-10">
              <p className="text-red-600 text-sm font-semibold uppercase tracking-wider mb-2">Customer Reviews</p>
              <h2 className="text-3xl font-bold text-gray-900">{sec.getTitle("testimonials") || "What Our Customers Say"}</h2>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
